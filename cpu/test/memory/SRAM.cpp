...
namespace memory {
    ...
    void SRAM::write(size_t address, uint32_t value) {
        assert((address+3) < memory.size());

        uint8_t* dest = &(memory.at(address));
    ...
    uint32_t SRAM::read32(size_t address) const {
        assert((address + 3) < memory.size());

        uint32_t data;
        const uint8_t* src = &(memory.at(address));
